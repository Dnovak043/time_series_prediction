...
| `configs/` | Generated experiment configs (`april_{nvda,intc,ibm}.yaml`) + `default.yaml` (carries the `data.asset_paths` catalog). Regenerated by every run — edits are overwritten. |
...
- **Quick sanity**: `python tests/test_pipeline_units.py` (seconds), then
  `april_smoke.ipynb` (1-day / 5-epoch pass through every stage including
  the GPU fan-out, ~15 min). Both are safe for Claude to run — no market
  data is needed by the unit tests.
- **The April experiment**: `april_run.ipynb` (Run All) or
  ...
  The 9 trainings are **fanned one per GPU** and reported in completion
  order; the schedule comes from `training.gpus` / `training.max_parallel`
  in the generated config (set them with `--gpus` / `--train-parallel`, or
  `GPUS` / `TRAIN_PARALLEL` in the notebook).
