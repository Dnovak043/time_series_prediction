...
(300 decodes for this scope); since featurize/encode are deterministic
(proven byte-for-byte by the parity and baseline harnesses), we compute
each distinct input once (16 decodes) — pure dedup of identical values.
...
Default scope: 20250401, symbol label INTC, his full 11 channels,
all 5 lengths x 5 classes = 25 ENS_TD files compared byte-for-byte.

Run:  .env/bin/python tests/verify_ensemble_reference.py           (~1h,
      mostly the reference side's 16 slow legacy featurize passes)
...
def run_reference(date: str, seq_lengths, class_names, data_dir: Path):
    ...
    args = (str(data_dir), date, "events", 100, [1, 2, 3, 4], t_start, t_end)

    print(f"[reference] featurize+encode {len(CHANNELS)} channels "
          "(his functions, fresh featurize per channel)...")
    z_list = []
    for predictor in CHANNELS:
        ...
        z_list.append(ref.get_bivariate_ts(ts, PREDICTED, predictor,
                                           0.05, 4))
        print(f"[reference]   {predictor}: {time.time()-t0:.0f}s", flush=True)

    cls_series = {}
    for cls in class_names:
        ts = ref.get_timeseries_by_date(SYMBOL, *args)
        cls_series[cls] = ref.add_class_label(ts, cls)
        print(f"[reference]   class {cls} labeled", flush=True)

    REF_DIR.mkdir(parents=True, exist_ok=True)
    for cls in class_names:
        daily_data = [(z_list, cls_series[cls])]
        for k in seq_lengths:
            ...
            # his naming, verbatim
            name = ("ENS_TD_" + SYMBOL + "_" + date[0:6] + "_SL_" + str(k)
...
def run_pipeline_stage(date: str, seq_lengths, class_names, data_dir: Path):
...
def main():
    ...
    ap.add_argument("--date", default="20250401")
    ...
    print(f"=== 1/2 reference (colleague's code) — {args.date}, "
          f"{len(seq_lengths)}x{len(class_names)} combos ===")
    t0 = time.time()
    run_reference(args.date, seq_lengths, class_names, data_dir)
    print(f"reference done in {time.time()-t0:.0f}s")

    print("\n=== 2/2 pipeline ensemble stage, same scope ===")
    t0 = time.time()
    run_pipeline_stage(args.date, seq_lengths, class_names, data_dir)
