...
# [vendoring fix 2] `plotting` is his local module, not in the repo (and a
# PyPI package of the same name shadows it); its symbols are only used by
# the viz helper plot_real_valued_features, off the pipeline's distribution
# path. The original vendored process_distributions.py comments this out too.
...
# [vendoring fix 1] the module-level experiment driver below ran on
# import, which would execute his whole March/April run inside any
# importer. Guarded so only a direct `python process_distributions_v2.py`
# runs it; the pipeline imports the pure functions above unchanged.
if __name__ == "__main__":
    ...
    if variate  == "multivariate":
        predictor = ['ofi_L10_norm_n',"micro_price",'vpin']
        predictor_name = 'ofi_L10_norm_n'+'-'+"micro_price"+'-'+'vpin'
