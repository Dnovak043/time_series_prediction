...
# [vendoring fix 1] removed: `from plotting import plt_ts, overlay_intraday_series`
# (leftover local module, not in the repo; per author it is not needed)
...
def estimate_subsequence_class_probabilities_01(seq, classes, num_classes, max_subsequence_length, non_zero_only = True):
    ...
    for length in range(1, max_subsequence_length + 1):
        ...
        for i in range(len(seq) - length + 1):
            subseq = tuple(seq[i:i + length])
            terminal_class = classes.iloc[i + length - 1]  # [vendoring fix 4] positional, as in baseline
            subseq_class_counts[subseq][terminal_class] += 1
...
#------------------------------------------------------------------------------
# [vendoring fix 2] removed plot_real_valued_features() (called the
# removed plotting module; per author it is not needed)
#------------------------------------------------------------------------------


def run_reference_driver():
    """Original module-level driver, unchanged (author's scope: AAPL,
    Windows path). Previously executed on import; now only runs when
    ...
    #------------------------------------------------------------------------------
    # [vendoring fix 3] removed the plot_ind_real_valued block (guard had a
    # typo `ind_real_valued` -- author confirmed -- and its only call was the
    # removed plotting helper)


    calculated = {}
    saved = {}
    for predictor in features[1:]:
        calculated[predictor] = False
        saved[predictor]      = False 
    ...
    for clsName in clsNames:
        ...
        for predictor in features[1:]:
            ...
            i = 0
            print('Start for Prerdictor', predictor, ' Predicted =', predicted, 'Class =',clsName)
            for date in dates:
                print('---------',date,'-------------')
                time_series = get_timeseries_by_date(symbol,fPath, date, resampling,frequency,frw_intervals ,tStart,tEnd )
                #--------------------------------------------------------------------------------------------------------------------
                if sequence_distribution_calculation and not calculated[predictor]:
                ...
                if class_calculation:
                    ...
                    cl_distributions=estimate_subsequence_class_probabilities(z_series12, cls_ts, max_subsequence_length= max_subsequence_length, class_values=(-1, 0, 1))
            ...
            if sequence_distribution_calculation and save_sequence_distributions and not saved[predictor]:                   # saving monthly aggregated distributions for training
            ...
            if class_calculation and save_class_distributions:
                outfname = 'CLS_DISTR_'+symbol+'_' + '_'+predicted+'-'+predictor+'_'+dates[0][:6]+'_'+ clsName
