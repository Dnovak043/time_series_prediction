...
#-----------------------------------------------------------------------------
# Generalized version of get_bivariate_ts
...
def get_z_ts(
    ...
    return_alphabet_size=False,
):
    """
    General z-symbol encoding.
    ...
    and for:
        ...
        predictor = ["vpin", "micro_price", "ofi_L10_norm_n"]

    Output is a one-dimensional integer pd.Series.
    """
    ...
    Z = (
        time_series_z[sym_cols]
        .astype(np.int64)
        .to_numpy()
    )

    weights = n_symbols ** np.arange(
        len(sym_cols),
        dtype=np.int64,
    )
    ...
    alphabet_size = n_symbols ** len(variables)
    ...
    if return_alphabet_size:
        return z_series, alphabet_size
    ...
    for seq_length in seq_lengths:
        for cls_name in cls_Names:
            ...
            for date in dates:
                ...
                date_ts = get_timeseries_by_date(symbol, fPath, date, resampling, frequency, 
                                                 frw_intervals, tStart, tEnd ) 
                z_series_list  = []
            
                for predictor in predictors:
                    z_ts =  get_z_ts(date_ts,  predicted, predictor, alpha, n_symbols)
                        
                    z_series_list.append(z_ts)
