...
def plotDistribution(Dist1, samples,  Title, Label1,  c1 = 'red', fName="bars"):           
    ...
    # Adding Xticks
    plt.xlabel('Sequences',  fontsize = 8)
    plt.ylabel('Probabilities', fontsize = 8)
...
def _dist_to_dict(dist):
    """
    Convert distribution from
        [[[0], 0.125], [[1], 0.124], [[0,1], 0.03], ...]
    to
        {(0,): 0.125, (1,): 0.124, (0,1): 0.03, ...}
    """
    out = {}
    for seq, prob in dist:
        out[tuple(seq)] = float(prob)
    return out


def _seq_to_label(seq):
    return "(" + ",".join(str(x) for x in seq) + ")"


def plot_distributions_comparison_preserve_order(
    distributions,
    names=None,
    colors=None,
    reference_index=0,          # which input distribution defines the sequence order
    top_n=None,                 # first top_n entries in input order; None = all
    explicit_indices=None,      # optional list of positions from the reference distribution
    figsize=(16, 6),
    ...
    """
    Compare multiple distributions while preserving the exact sequence order
    of the reference input distribution.

    Important:
    - No sorting is applied.
    - Zero-probability sequences are kept if they exist in the reference input.
    - Other distributions are aligned to the same sequence list; if a sequence
      is missing there, its probability is taken as 0.
    """
    ...
    if len(names) != n_dists:
        raise ValueError("names must have the same length as distributions")

    if colors is None:
        default_colors = ["black", "red", "blue", "green", "purple", "orange"]
        colors = default_colors[:n_dists]
    if len(colors) != n_dists:
        raise ValueError("colors must have the same length as distributions")

    # Dictionaries for alignment
    dist_dicts = [_dist_to_dict(d) for d in distributions]

    # ------------------------------------------------------------------
    # Use the reference distribution order exactly as given in the input
    # ------------------------------------------------------------------
    ref_dist = distributions[reference_index]
    ref_sequences = [tuple(seq) for seq, _ in ref_dist]

    # Optional sub-selection by position
    if explicit_indices is not None:
        selected = [ref_sequences[i] for i in explicit_indices]
    elif top_n is not None:
        selected = ref_sequences[:top_n]
    else:
        selected = ref_sequences

    # ------------------------------------------------------------------
    # Build aligned probability matrix
    # ------------------------------------------------------------------
    prob_matrix = np.zeros((n_dists, len(selected)), dtype=float)
    for i, d in enumerate(dist_dicts):
        for j, seq in enumerate(selected):
            prob_matrix[i, j] = d.get(seq, 0.0)

    # ------------------------------------------------------------------
    # Plot
    # ------------------------------------------------------------------
    x = np.arange(len(selected))
    width = bar_group_width / n_dists

    fig, ax = plt.subplots(figsize=figsize)

    for i in range(n_dists):
        xpos = x - bar_group_width / 2 + (i + 0.5) * width
        ax.bar(
            xpos,
            prob_matrix[i],
            width=width,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [_seq_to_label(s) for s in selected],
        rotation=rotation,
    ...
    return fig, ax, selected, prob_matrix
