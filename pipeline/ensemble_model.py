...
def encoder_weights_path(cfg: RunConfig, channel: int,
    ...
    variate = ("bivariate" if isinstance(em.channels[channel], str)
               else "multivariate")
    name = (f"WGHTS_{cfg.data.symbol}_{variate}_"
            f"{cfg.distributions.predicted}-{em.channel_names[channel]}_"
            f"{_month(cfg)}_{em.channel_qubits[channel]}q.pt")
